...
describe('root store', () => {
  ...
  it('should hold strongly typed ref to transfer store', () => {
    expect(rootStore).toHaveProperty('transferStore');
    expect(rootStore.transferStore).toBeDefined();
    expect(rootStore.transferStore).toBeInstanceOf(TransferStore);
  });
});
